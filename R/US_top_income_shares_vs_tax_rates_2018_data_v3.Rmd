...
# US_top_income_shares_18 <- read.xlsx("../data/TabFig2018prel.xlsx", sheetIndex = 9, sheetName = "Table A3", startRow = 6, endRow = 108, colIndex = c(1:7, 9:13), header = FALSE)
US_income_averages_18 <- read.xlsx("../data/TabFig2018prel.xlsx", 
...
top_income_tax <- cbind(US_top_income_shares_18[c("Year", "P99_100")], US_income_averages_18[c("Bottom_99", "Top_1", "Bottom_99_K", "Top_1_K")], tax_rates[2:3])
Rate_99 <- US_income_averages_18[, "Bottom_99"] / US_income_averages_18[, "Bottom_99"][1] * 100
Rate_1 <- US_income_averages_18[, "Top_1"] / US_income_averages_18[, "Top_1"][1] * 100
Rate_99_K <- US_income_averages_18[, "Bottom_99_K"] / US_income_averages_18[, "Bottom_99_K"][1] * 100
Rate_1_K <- US_income_averages_18[, "Top_1_K"] / US_income_averages_18[, "Top_1_K"][1] * 100
```
...
M_tbl <- data.frame(top_income_tax[c("Year", "P99_100_z", "Marginal_z")]) %>%
  as_tibble %>%
...
M_melt <- melt(data.frame(top_income_tax[c("Year", "P99_100_z", "Marginal_z")]),
       ...
       value.name = "Values")
str(M_melt)
Rates_tbl <- data.frame(top_income_tax[c("Year", "Marginal_z")], Rate_99_z, Rate_1_z) %>%
  as_tibble %>%
  gather(key = "Variables", value = "Values", -Year) %>%
  mutate(Variables = factor(Variables))
str(Rates_tbl)
Rates_melt <- melt(data.frame(top_income_tax[c("Year", "Marginal_z")], Rate_99_z, Rate_1_z),
       ...
       value.name = "Values")
str(Rates_melt)
Rates_K_tbl <- data.frame(top_income_tax[c("Year", "Marginal_z")], Rate_99_K_z, Rate_1_K_z) %>%
  as_tibble %>%
  gather(key = "Variables", value = "Values", -Year) %>%
  mutate(Variables = factor(Variables))
str(Rates_K_tbl)
